...
def filledStack(mazeMap):
    ...
    to be used w/ the function Dijkstra()
...
def dists_from_each(locations, maze_map):
    ...
    the fist parameter is a dictionnary
    - tuple with all the dists
    the second parameter is a dict
    - for each tuple return a list
    TODO :
...
def update_dists_from_each(dists_matrix, routes_matrix, new_location, maze_map, coins):
    ...
    should take as parameter :
    dists_matrix : the generated dists_matrix
    routes_matrix : the generated routes_matrix
    maze_map : to update new locations
    """
...
def direction(old, new):
    ...
    it can bug if the direction is not safe
    In this case the IA will crash...
    """
    if new[0] - old[0] == -1:
        return UP
    elif new[0] - old[0] == 1:
...
def dijkstra(mazeMap, start):
    ...
    uses MinStack wich is a custom stack, 
    it is strangely faster than the normal implementation...
    """
    waiting = MinStack()
    routing = {}
    waiting.add(start,0)
...
def way_width(routing, start, end):
    ...
    this one is in the right order :
    [start, ..., ..., end]
...
def fill_packages(coins, mazeMap):
    ...
    should be called once, it is very slow
    """
